...
export default async function HomePage() {
  // Public UI centers on the Reference Library — force `pdf` as the primary
  // public task on the homepage.
  const pdfTask = SITE_CONFIG.tasks.find((t) => t.key === 'pdf' && t.enabled)
  const fallbackTask = SITE_CONFIG.tasks.find((task) => task.enabled)
  const primaryTask = (pdfTask?.key || fallbackTask?.key || 'pdf') as TaskKey
